...
h2 {
    ...
    color: var(--shopee-dark) !important;
...
/* Caption di bawah title */
.stApp [data-testid="stCaptionContainer"] p {
    color: var(--shopee-subtext) !important;
...
section[data-testid="stSidebar"] [data-testid="stHeading"] p,
section[data-testid="stSidebar"] [data-testid="stHeading"] span {
    color: white !important;
    font-weight: 700 !important;
}
...
.stError {
    ...
    border-left: 4px solid var(--shopee-orange) !important;
    border-radius: var(--radius-sm) !important;
}
...
[data-testid="stDataFrame"] {
    border-radius: var(--radius) !important;
    overflow: hidden;
    border: 1px solid var(--shopee-orange-mid) !important;
    box-shadow: var(--shadow-sm);
}
...
/* ── DIVIDER ── */
hr {
    border-color: var(--shopee-orange-mid) !important;
...
.stTabs [data-baseweb="tab-list"] {
    ...
    padding: 4px;
    gap: 4px;
    border: 1px solid var(--shopee-orange-mid);
}
...
.sidebar-section {
    ...
    color: var(--shopee-orange) !important;
    padding: 6px 0 4px 0;
    border-bottom: 2px solid var(--shopee-orange-mid);
    margin-bottom: 6px;
}

/* ── TOP HEADER BAR ── */
.top-header {
    background: linear-gradient(135deg, var(--shopee-orange) 0%, var(--shopee-orange-2) 100%);
    border-radius: var(--radius);
    padding: 18px 28px;
    margin-bottom: 20px;
    display: flex;
...
with st.sidebar:
    ...
    <div style="background:linear-gradient(135deg,#EE4D2D,#F85606);
        ...
        <div style="color:white; font-family:'Nunito',sans-serif;
                    font-weight:900; font-size:1rem;">⚙️ Parameter Simulasi</div>
        <div style="color:rgba(255,255,255,0.8); font-size:0.75rem; margin-top:2px;">
            Atur semua parameter di bawah ini
        </div>
    </div>
    """, unsafe_allow_html=True)

    # — Titik Demand & Kandidat —
...
info = STATUS_INFO.get(solver_status, {
    "fn": st.warning,
    "msg": f"⚠️ Status solver: **{solver_status}**."
})
info["fn"](info["msg"])
...
# ── TABS: PETA + TABEL + BIAYA ─────────────────────────────────────────────────
tab1, tab2, tab3 = st.tabs(["🗺️  Peta Distribusi", "📋  Analisis Gudang", "📊  Breakdown Biaya"])
